...
export class CloudUploadCoordinator {
	...
	async ensureDropboxClient(token, options = {}) {
		...
		const oauthRecord = (this.session && this.session.dropboxOAuth) || (typeof session !== "undefined" ? session.dropboxOAuth : null) || null;
		const oauthFresh = Boolean(oauthRecord && (!oauthRecord.expiresAt || Date.now() < oauthRecord.expiresAt));
	...
	async uploadBlob(blob, options = {}) {
		...
		const { filename, drive = true, dropbox = true, onProgress, signal, driveChunkSize = DEFAULT_DRIVE_CHUNK_SIZE, dropboxChunkSize = DEFAULT_DROPBOX_CHUNK_SIZE } = options;

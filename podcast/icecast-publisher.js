...
async function getAacEncoderConfig(audioContext, audioBitsPerSecond) {
	if (typeof AudioEncoder === "undefined" || typeof AudioEncoder.isConfigSupported !== "function" || typeof MediaStreamTrackProcessor === "undefined") {
		return null;
	}
	const config = {
		codec: "mp4a.40.2",
...
export class IcecastPublisher extends EventTarget {
	...
	async start(config = {}) {
		...
		const audioBitsPerSecond = Number.isFinite(config.audioBitsPerSecond) ? Math.max(16000, config.audioBitsPerSecond) : DEFAULT_AUDIO_BITRATE;
	...
	async stop({ quiet = false } = {}) {
		...
		await Promise.race([Promise.all([uploadSettled, encoderSettled]), delay(4000)]);
	...
	addSourceNode(key, track) {
		try {
			...
			this.emitStatus("live", `Icecast live (${this.sourceNodes.size} source${this.sourceNodes.size === 1 ? "" : "s"}).`);
		} catch (error) {
